"""
Template du scraper autonome (Playwright pur, sans LLM) genere pour un
site donne, et fonction qui l'ecrit sur disque a partir des selecteurs
deduits pendant l'exploration.
"""

import re

SCRAPER_TEMPLATE = '''"""
Scraper autonome généré automatiquement pour : {url}
Aucun LLM requis. Basé sur Playwright uniquement.
Utilisation : python {filename} "nom du produit à chercher"
"""
...
SELECTORS = {{
    "cookie_button": {cookie_selector!r},
    "search_input": {search_input_selector!r},
    "search_button": {search_button_selector!r},
    "price": {price_selector!r},
    "stock": {stock_selector!r},
}}
...
def find_price_generic(page):
    js = r"""
    () => {{
        const re = /(€|\\$|£)\\s?\\d+([.,]\\d{{1,2}})?|\\d+([.,]\\d{{1,2}})?\\s?(€|\\$|£)|\\d+([.,]\\d{{1,2}})?\\s?(kr|zl|z\\u0142|kc|k\\u010d|kn|ft|lei|chf|lv|\\u043b\\u0432)\\b/i;
...
def scrape(product_name: str) -> dict:
    with sync_playwright() as p:
        ...
        PRICE_RE = _re.compile(
            r"(€|\\$|£)\\s?\\d+([.,]\\d{{1,2}})?"
            r"|\\d+([.,]\\d{{1,2}})?\\s?(€|\\$|£)"
            r"|\\d+([.,]\\d{{1,2}})?\\s?(kr|zl|z\\u0142|kc|k\\u010d|kn|ft|lei|chf|lv|\\u043b\\u0432)\\b",
            _re.I,
        )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python {filename} \\"nom du produit\\"")
        sys.exit(1)
...
def generate_scraper(url: str, domain: str, selectors: dict) -> str:
    filename = f"scraper_{re.sub(r'[^a-zA-Z0-9]', '_', domain)}.py"
    content = SCRAPER_TEMPLATE.format(
        url=url,
        domain=domain,
        filename=filename,
        url_pattern=selectors["url_pattern"],
        cookie_selector=selectors["cookie"],
        search_input_selector=selectors["search_input"],
        search_button_selector=selectors["search_button"],
        price_selector=selectors["price"],
        stock_selector=selectors["stock"],
    )
