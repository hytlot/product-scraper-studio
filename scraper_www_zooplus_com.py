"""
Scraper autonome généré automatiquement pour : https://www.zooplus.com
Aucun LLM requis. Basé sur Playwright uniquement.
Utilisation : python scraper_www_zooplus_com.py "nom du produit à chercher"
"""

import sys
from playwright.sync_api import sync_playwright

SITE_URL = "https://www.zooplus.com"
DOMAIN = "www.zooplus.com"
URL_PATTERN = '__SLUG__'

SELECTORS = {
    "cookie_button": '',
    "search_input": '#search_query_field_desktop',
    "search_button": 'button[type="submit"]',
    "price": 'div.z-product-price__price-wrap',
    "stock": '',
}
...
def find_price_generic(page):
    js = r"""
    () => {
        const re = /(€|\$|£)\s?\d+([.,]\d{1,2})?|\d+([.,]\d{1,2})?\s?(€|\$|£)|\d+([.,]\d{1,2})?\s?(kr|zl|z\u0142|kc|k\u010d|kn|ft|lei|chf|lv|\u043b\u0432)\b/i;
...
def scrape(product_name: str) -> dict:
    with sync_playwright() as p:
        ...
        import re as _re
        # Regex de prix robuste : decimales OPTIONNELLES dans tous les cas
        # (couvre les prix ronds sans decimales, ex. suedois "699 kr"),
        # et plusieurs devises courantes en plus de €/$/£/kr.
        PRICE_RE = _re.compile(
            r"(€|\$|£)\s?\d+([.,]\d{1,2})?"
            r"|\d+([.,]\d{1,2})?\s?(€|\$|£)"
            r"|\d+([.,]\d{1,2})?\s?(kr|zl|z\u0142|kc|k\u010d|kn|ft|lei|chf|lv|\u043b\u0432)\b",
            _re.I,
        )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python scraper_www_zooplus_com.py \"nom du produit\"")
        sys.exit(1)
